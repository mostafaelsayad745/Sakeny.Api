...
namespace sakeny.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetUser(int id , bool includePostFeedbacks)
        {
            ...
            {
                var userWithPostFeedbacks = _mapper.Map<UserForReturnWithPostFeedbacks>(userFromRepo);
                return Ok(userWithPostFeedbacks);
            }
        ...
        [HttpPut("{userid}")]
        public IActionResult UpdateUser(int userid, UserForUpdateDto userForUpdateDto)
        {
            var db = new HOUSE_RENT_DBContext();
        ...
        [HttpPatch("{userid}")]
        public IActionResult PartialUpdateUser(int userid, JsonPatchDocument< UserForUpdateDto> patchDocument)
        {
            var db = new HOUSE_RENT_DBContext();
            var userToUpdate = db.UsersTbls.FirstOrDefault(u => u.UserId == userid);
            if (userToUpdate == null)
        ...
        {
            var db = new HOUSE_RENT_DBContext();
            var userToDelete = db.UsersTbls.FirstOrDefault(u => u.UserId == userid);
            if(userToDelete == null)
            {
                return NotFound();
            }
            db.UsersTbls.Remove(userToDelete);
            db.SaveChanges();
            return NoContent();
        }
    }
